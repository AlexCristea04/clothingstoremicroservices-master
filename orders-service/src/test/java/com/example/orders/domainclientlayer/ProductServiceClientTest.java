...
import com.example.orders.domainclientlayer.products.ProductModel;
...
public class ProductServiceClientTest {
    ...
    public void testGetProductByProductId() {
        ...
        ProductModel expectedModel = new ProductModel("", "", "", "", new BigDecimal("0"), 0, "");

        // Mocking successful retrieval
        when(restTemplate.getForObject(url, ProductModel.class)).thenReturn(expectedModel);

        // Act
        ProductModel result = productServiceClient.getProductByProductId(productId);

        // Assert
        assertNotNull(result);
        verify(restTemplate, times(1)).getForObject(url, ProductModel.class);
    }
    ...
    public void testGetProductByProductId_NotFound() throws IOException {
        ...
        when(restTemplate.getForObject(url, ProductModel.class)).thenThrow(ex);
