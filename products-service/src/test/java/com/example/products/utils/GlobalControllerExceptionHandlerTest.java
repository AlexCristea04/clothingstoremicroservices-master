...
import com.example.products.utils.exceptions.ProductNotFoundException;
import com.example.products.utils.exceptions.InUseException;
import com.example.products.utils.exceptions.InvalidInputException;
...
class GlobalControllerExceptionHandlerTest {
    ...
    void handleCustomerNotFoundExceptionWithMessageAndCause() {
        ...
        ProductNotFoundException ex = new ProductNotFoundException(message, cause);
        WebRequest request = mock(WebRequest.class);
        HttpStatus expectedStatus = HttpStatus.NOT_FOUND;

        assertEquals(expectedStatus, handler.handleProductNotFoundException(request, ex).getHttpStatus());
        assertNotNull(handler.handleProductNotFoundException(request, ex).getMessage());
    }

    @Test
    void handleCustomerNotFoundExceptionWithCause() {
        Throwable cause = new Throwable("Cause of employee not found");
        ProductNotFoundException ex = new ProductNotFoundException(cause);
        WebRequest request = mock(WebRequest.class);
        HttpStatus expectedStatus = HttpStatus.NOT_FOUND;
    ...
    void handleCustomerNotFoundExceptionNoMessage() {
        ...
        assertEquals(expectedStatus, handler.handleProductNotFoundException(request, ex).getHttpStatus());
        assertNull(handler.handleProductNotFoundException(request, ex).getMessage());
    }
